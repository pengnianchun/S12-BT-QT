...
bool MainWindow::DeviceConfig(void)
{
    ...
    QString cmdStr[]={"CmdFaild","OnlineCheck","EraseFlash","SetBaudRate","BlockWriteInfo",
                      "WriteBlockFlash","BlockReadInfo","ReadBlockFlash","ExcuteApp","CmdSuccess"};
    uint8_t cmdData[16];
    for(int i=0;i<ui->cmdListTableWidget->rowCount();i++){
...
//    }
    ...
    if (firmwareFile.open(QFile::ReadOnly)){
        ...
        while (!in.atEnd()){
            line = in.readLine();
            qDebug()<<" --- Read file line string = "<<line;
        }
        firmwareFile.close();
#else
        if(!ui->allNodeCheckBox->isChecked()){
...
void MainWindow::sendData(int ID, char* msg, uint length) {
    int DevType = 4; //USB-CAN2
    int DevIndex = 0;
    int CANIndex = ID;
    PVCI_CAN_OBJ CAN_Data = (PVCI_CAN_OBJ)msg;
    CAN_Data->ID  = ID;
    CAN_Data->SendType   = 0;
    CAN_Data->RemoteFlag = 0;
    CAN_Data->ExternFlag = 0;
    CAN_Data->DataLen = length;
